...
이미지에서 막히는 건 CORS 때문입니다. Figma의 capture.js는 이미지를 캔버스로 읽는데, CORS 헤더가 없는 다른 도메인 CDN 이미지는 읽지 못해서 빈칸으로 들어옵니다. 그래서 이미지 주소를 CORS를 열어주는 프록시(wsrv.nl)로 바꿔치기하고, 프록시가 실패하면 원본으로 되돌립니다. 다른 도메인의 CSS 배경 이미지(background-image)도 같은 방식으로 바꿉니다. 반면 스크린샷 방식은 브라우저가 이미 그린 화면을 통째로 뜨기 때문에 CORS와 상관없이 이미지가 다 나옵니다.

아이콘에서도 자주 막힙니다. 많은 사이트가 아이콘을 그림이 아니라 "아이콘 전용 글꼴(아이콘 폰트)"의 글자로 그립니다. 이런 아이콘은 이미지가 아니라서 프록시로는 못 잡고, Figma에는 그 글꼴이 없어 네모나 빈칸으로 들어옵니다. 그래서 캡처 직전에 각 아이콘 글자를 그 글꼴로 캔버스에 직접 그려 PNG로 만든 뒤, 진짜 이미지로 끼워 넣습니다. 그러면 capture.js가 평범한 이미지 레이어로 가져갑니다. 이것도 스크린샷 방식에서는 신경 쓸 필요가 없습니다.
...
브라우저에서 돌리는 스크립트는 scripts/ 폴더에 있고, evaluate_script의 function 인자에 파일 내용을 그대로 넣습니다.
...
3. (선택) scripts/diagnose.js를 실행해 무엇이 빠질지 미리 봅니다. 아이콘 폰트나 배경 이미지가 있는지 알려줍니다. 읽기만 하고 페이지는 바꾸지 않습니다.
4. scripts/1-prep-images.js를 실행합니다. 끝까지 스크롤해 lazy 이미지를 다 띄우고, 모든 img와 다른 도메인 배경 이미지를 wsrv.nl 프록시로 바꾸고, 실패분은 한 번 더 시도한 뒤 원본으로 되돌립니다.
5. scripts/2-bake-icon-fonts.js를 실행합니다. 아이콘 폰트로 그린 아이콘을 PNG 이미지로 구워 끼워 넣습니다. baked가 0이면 이 사이트엔 아이콘 폰트가 없는 것이니 그냥 넘어갑니다.
6. scripts/3-inject-capture.js를 실행해서 capture.js를 주입합니다.
7. scripts/4-run-capture.js를 실행합니다. CAPTURE_ID를 받은 값으로 바꿉니다. Runtime.callFunctionOn timed out이 떠도 전송은 성공한 경우가 많으니 9번에서 확인합니다.
8. generate_figma_design(fileKey, captureId)를 status가 completed가 될 때까지 반복합니다. 최대 열 번 정도 보고, 끝나면 node-id가 옵니다.
...
아이콘이 빠진다면 십중팔구 "아이콘 웹폰트"입니다. 아이콘이 그림이 아니라 글꼴의 글자라서 이미지 프록시로는 안 잡히고, Figma에 그 글꼴이 없어 네모나 빈칸이 됩니다. 2-bake-icon-fonts.js가 아이콘을 이미지로 구워서 해결합니다. baked 수가 0이면 그 사이트엔 아이콘 폰트가 없는 것이니 넘어가면 됩니다.

페이지를 이동하면(SPA 라우트 변경) DOM이 초기화됩니다. 그러면 이미지 준비(1단계)부터 다시 해야 합니다. capture.js가 띄우는 기본 재캡처 버튼은 이 준비 과정을 안 거치니, 그 버튼 말고 반드시 이 워크플로우로 돌려야 이미지와 아이콘이 삽니다.

스크립트 결과의 url이 about:blank로 나오거나 이미지 수가 0이면 페이지가 리셋된 것입니다. navigate부터 다시 하세요.

wsrv.nl 프록시는 한꺼번에 많은 이미지를 요청하면 일부를 놓쳐 결과가 매번 조금씩 다릅니다. 1-prep-images.js가 실패분을 한 번 더 시도하지만, 그래도 안 되는 까다로운 이미지(복잡한 쿼리가 붙은 webp 등)는 원본으로 되돌아가 캡처에는 빈칸이 됩니다. 그런 이미지가 중요하면 B 방식을 같이 쓰세요.

가상 스크롤을 쓰는 페이지는 화면 밖 요소가 DOM에 없어서 일부가 빠질 수 있습니다. shadow DOM이나 iframe 안의 내용도 일부 누락될 수 있습니다.

captureId는 한 번만 씁니다. 페이지마다 새로 받으세요. 무료 Figma 팀은 호출 제한이 있으니 org나 유료 시트 파일에서 작업하세요.
...
web-to-figma/
  ...
  skills/
    web-to-figma/
      SKILL.md               스킬 정의
      scripts/
        diagnose.js          무엇이 빠질지 미리 진단 (선택, 읽기 전용)
        1-prep-images.js     스크롤, CORS 프록시 교체와 재시도, 배경 이미지 처리 (A 방식)
        2-bake-icon-fonts.js 아이콘 웹폰트를 이미지로 굽기 (A 방식)
        3-inject-capture.js  capture.js 주입
        4-run-capture.js     captureForDesign 실행
...
The remaining problem is CORS. Figma capture.js reads each image through a canvas, so cross-origin CDN images without CORS headers come back blank. The fix is to swap every img (and cross-origin CSS background-image) to a CORS-enabled proxy (wsrv.nl) and fall back to the original URL when the proxy fails. Icons are a second trap: many sites draw them as glyphs of an icon webfont rather than as images, so the proxy cannot help and Figma renders them as empty boxes because it lacks the font. The workflow bakes each icon glyph onto a canvas with its own font and injects it as a real PNG image so capture.js picks it up as an ordinary image layer. A full-page screenshot avoids both problems because the browser has already painted the pixels.

There are two modes. Mode A produces editable Figma layers: get a captureId from generate_figma_design, navigate with chrome-devtools, run prep-images (scroll to the bottom, proxy swap with a retry pass, fallback), bake icon webfonts to images, inject capture.js, call window.figma.captureForDesign, then poll generate_figma_design until it reports completed. Mode B produces a flat screenshot: navigate, scroll to load images, take_screenshot with fullPage, then POST the PNG through upload_assets.
...
A few things to keep in mind. Re-run image prep after any in-app navigation, since SPA route changes reset the DOM; if a script returns url about:blank or zero images, the page reset and you should navigate again. If icons go missing it is almost always an icon webfont, which 2-bake-icon-fonts.js handles. The wsrv.nl proxy is flaky under load and drops some images, so prep retries failures once; stubborn ones (complex-query webp) fall back to the original and appear blank in Mode A, so run Mode B alongside if you need every image. Virtualized lists, shadow DOM, and iframes may drop content. The captureId is single use per page. A CDP Runtime.callFunctionOn timeout on the capture call usually still submits, so verify by polling instead of assuming it failed.
